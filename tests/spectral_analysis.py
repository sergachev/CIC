...
class TB(object):
    ...
    async def generate_input(self, num_items):
        ...
        if True:
            ...
            values = np.round((signal_amplitude * np.sin(phases) + signal_amplitude * np.sin(phases2) + noise) * (
                        2 ** (self.INP_DW - 1) - 1))
        if False:
    ...
    for rate in rate_list:
        ...
        if True:
            ...
            if True:
                fig1 = plt.figure()
                plt.title(
                    F"Power Spectrum of input\nf_clk = {tb.f_clk * 1E-6} MHz, n = {num_items}, window = {window_text}")
                (PSD_input, freq) = PSD(np.array(tb.input) / (2 ** (tb.INP_DW - 1) - 1), window=window_text,
                                        fs=tb.f_clk, sides='one', scaling=scaling)
            ...
            fig3 = plt.figure()
            plt.title(
                F"Power Spectrum of output\nf_clk = {tb.f_clk * 1E-6} MHz, n = {num_items}, window = {window_text}")
            output_normalized = np.array(output) / (2 ** (tb.OUT_DW - 1) - 1)
            if False:
                ftype = signal.dlti(signal.firwin(200 * tb.R + 1, 1. / tb.R, window='hamming'), [1])
            if True:
                ftype = 'iir'
            fir_decimated_normalized = signal.decimate(np.array(tb.input) / (2 ** (tb.INP_DW - 1) - 1), tb.R,
                                                       ftype=ftype)
            (ydata_onesided, freq) = PSD(output_normalized, window=window_text, fs=tb.f_clk / tb.R, sides='one',
                                         scaling=scaling)
            (ydata_ideal_onesided, freq_ideal) = PSD(fir_decimated_normalized, window=window_text, fs=tb.f_clk / tb.R,
                                                     sides='one', scaling=scaling)
